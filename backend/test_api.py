...
def run_tests():
    ...
    # 1. Test Ping
    print("\n1. Testing GET /ping...")
    status, res = make_request(f"{API_PREFIX}/ping", "GET")
    ...
    status, res = make_request(f"{API_PREFIX}/register", "POST", data=admin_payload)
    print(f"Status: {status} | Response: {res}")
    ...
    status, res = make_request(f"{API_PREFIX}/register", "POST", data=user_payload)
    print(f"Status: {status} | Response: {res}")
    ...
    status, res = make_request(f"{API_PREFIX}/login", "POST", data=login_payload)
    ...
    status, res = make_request(f"{API_PREFIX}/users/me", "GET", headers=user_headers)
    ...
    status, res = make_request(
        f"{API_PREFIX}/users/{user_id}/status", "GET", headers=user_headers
    )
    print(f"Status: {status} | Response: {json.dumps(res, indent=2)}")
    ...
    status, res = make_request(f"{API_PREFIX}/users/all", "GET", headers=user_headers)
    print(f"Status: {status} | Response: {res}")
    ...
    status, admin_res = make_request(
        f"{API_PREFIX}/login", "POST", data=admin_login_payload
    )
    ...
    status, res = make_request(f"{API_PREFIX}/users/all", "GET", headers=admin_headers)
    print(f"Status: {status} | Response: {json.dumps(res, indent=2)}")
    ...
    status, res = make_request(
        f"{API_PREFIX}/token/refresh", "POST", data=refresh_payload
    )
    print(f"Status: {status} | Response keys: {list(res.keys())}")
    ...
    status, res = make_request(
        f"{API_PREFIX}/token/refresh", "POST", data=logout_payload
    )
    print(f"Status: {status} | Response: {res}")
    ...
    status, res = make_request(
        f"{API_PREFIX}/users/{user_id}/status", "GET", headers=admin_headers
    )
    print(f"Status: {status} | Response: {json.dumps(res, indent=2)}")
    assert status == 200, "Status query failed"
...
if __name__ == "__main__":
    ...
    try:
        req = urllib.request.Request(
            f"http://127.0.0.1:8005{API_PREFIX}/ping", method="GET"
        )
        with urllib.request.urlopen(req, timeout=1.0) as conn:
            if conn.read().decode("utf-8") == "pong":
                server_already_running = True
                print(
                    "Detected uvicorn server already running on port 8005. Running tests against it..."
                )
    except Exception:
        pass
    ...
    if not server_already_running:
        ...
        for _ in range(15):
            ...
            try:
                req = urllib.request.Request(
                    f"{API_URL}{API_PREFIX}/ping", method="GET"
                )
                with urllib.request.urlopen(req, timeout=1.0) as conn:
                    if conn.read().decode("utf-8") == "pong":
                        server_ready = True
